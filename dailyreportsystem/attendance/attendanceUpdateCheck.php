...
if (!isset($_SESSION['attendanceEdit'])) {
    header('Location: ../login/login-index.php');
    exit();
}

if (!empty($_POST)) {
    $account = $db->prepare('UPDATE attendance SET att=?, lea=?, year=?, month=?, day=?, week=? WHERE attenId=?');
    $account->execute(array(
        $_SESSION['attendanceEdit']['att'],
        $_SESSION['attendanceEdit']['lea'],
        $_SESSION['attendanceEdit']['year'],
        $_SESSION['attendanceEdit']['month'],
        $_SESSION['attendanceEdit']['day'],
        $_SESSION['attendanceEdit']['week'],
        $_SESSION['attendanceEdit']['attenId']
    ));
    header('Location: attendanceUpdateFinish.php');
    exit();
} 
    ...
    <body>
        ...
        　</header>
          <div class="top-wrapper">
              <p class="site-title-sub">勤怠編集確認</p>
          </div>
