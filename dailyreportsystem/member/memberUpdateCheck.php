...
if (!isset($_SESSION['memberEdit'])) {
    header('Location: ../login/login-index.php');
    exit();
}

if (!empty($_POST)) {
    $account = $db->prepare('UPDATE member SET nam=?, kana=?, pos=?, log=?, pass=? WHERE id=?');
    $account->execute(array(
        $_SESSION['memberEdit']['nam'],
        $_SESSION['memberEdit']['kana'],
        $_SESSION['memberEdit']['pos'],
        $_SESSION['memberEdit']['log'],
        $_SESSION['memberEdit']['pass'],
        $_COOKIE['userId']    
    ));
    unset($_SESSION['memberEdit']);
    header('Location: memberUpdateFinish.php');
    exit();
} 
          ...
          <div class="join-wrapper">
                <form action="" method="post">
                    <input type="hidden" name="action" value="submit"/>    
                        ...
                        <p>パスワード</p> 
                        <p>********</p>    
                        <a href="member-edit.php?action=rewrite">&laquo;&nbsp;中止する</a> | <input type="submit" value="編集する" />
              </form>  
              </div>
          <footer>    
            <p>Example Co. Ltd.</p>
